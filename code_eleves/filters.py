...
class Lin2d(nn.Module):
    ...
    def __init__(self, x_dim, N, dt, init,
        ...
        # implement M
        theta = np.pi/100
        self.M = torch.tensor([[np.cos(theta), np.sin(theta)],[-np.sin(theta), np.cos(theta)]])
...
class Gaussian(nn.Module):
    ...
    def __init__(self, x_dim, vec, inds):
        ...
        diaga = torch.max(torch.min(diaga, maxexp), minexp)
    ...
    def log_prob(self, x):
        ...
        if slow:
            ...
            for i in range(bs):
                ...
                term_1 = -0.5 * torch.mm(mean_diff.unsqueeze(0), torch.mm(cov_inv, mean_diff.unsqueeze(1))) #ptet inverser les torch.mm
            ...
            #term_1 = -0.5 * torch.bmm(mean_diff.unsqueeze(2).transpose(1, 2), torch.bmm(cov_inv, mean_diff.unsqueeze(2))).squeeze(2)
    ...
    #def log_prob(self, x):
    #    # Compute log probability using self.mu and self.Lambda
    #    bs = x.shape[0]
    #    mean_diff = x - self.mu
    #    cov_inv = self.cov_inv_
    #    term_1 = -0.5 * torch.bmm(mean_diff.unsqueeze(2).transpose(1, 2), torch.bmm(cov_inv, mean_diff.unsqueeze(2))).squeeze(2)
